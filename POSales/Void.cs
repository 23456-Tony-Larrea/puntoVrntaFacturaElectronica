    ...
    public partial class Void : Form
    {
        ...
        private void btnVoid_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Anulado por nombre y cancelado por nombre son lo mismo!. Por favor anule por otra persona.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                string user;
                cn.Open();
                ...
                if (dr.HasRows)
                {
                    ...
                    {
                        dbcon.ExecuteQuery("UPDATE Productos SET cantidad = cantidad + " + cancelOrder.udCancelQty.Value + " where codigo= '" + cancelOrder.txtPcode.Text + "'");
                    }
                    dbcon.ExecuteQuery("UPDATE Carrito SET cantidad = cantidad + " + cancelOrder.udCancelQty.Value + " where id LIKE '" + cancelOrder.txtId.Text + "'");
                    MessageBox.Show("Transacción de pedido cancelada con éxito!", "Orden Cancelada", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.Dispose();
        ...
        public void SaveCancelOrder(string user)
        {
            ...
            {
                cn.Open();
                cm = new SqlCommand("insert into Cancel (transno, pcode, price, qty, total, sdate, voidby, cancelledby, reason, action) values (@transno, @pcode, @price, @qty, @total, @sdate, @voidby, @cancelledby, @reason, @action)", cn);
